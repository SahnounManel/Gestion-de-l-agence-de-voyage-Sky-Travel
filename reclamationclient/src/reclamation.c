...
{f=fopen("reclamation.txt","a+");
	while(fscanf(f,"%s %s %s %s %s \n",ID_client,ID_reclamation,prestation,date_reclamation,reclamation)!=EOF)
	{
	gtk_list_store_append(store,&iter);
	gtk_list_store_set(store,&iter,ID_RECLAMATION,ID_reclamation,DATE_RECLAMATION,prestation,PRESTATION,date_reclamation,-1);
	}
	fclose(f);
...
while(!feof(fichierIn)){
    ...
    strcpy(ID_Reclamation,"suppression_effectuee_avec_succes");
